	...
	tests := []vmTestCase{
		{
			input: `
	let returnsOne = fn() { 1; };
	let returnsOneReturner = fn() { returnsOne; }; returnsOneReturner()();
	`,
			expected: 1,
		},
		{
...
func TestIntegerArithmatic(t *testing.T) {
	tests := []vmTestCase{
		// input and what should be expected at the top of the stack after vm execution
		{"1", 1},
		{"2", 2},
		{"1 + 2", 3},
		{"1 - 2", -1},
