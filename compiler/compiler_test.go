	...
	tests := []compilerTestCase{
		{
			...
			expectedConstants: []interface{}{
				[]code.Instructions{
					...
					// get `a` since it's used in the inner function
					code.Make(code.OpGetLocal, 0),
...
func TestHashLiterals(t *testing.T) {
	tests := []compilerTestCase{
		...
		},
		{
			...
			expectedInstructions: []code.Instructions{
				/*
					OpHash specifies that the last 6 constants are key:value
					pairs
				*/
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
	...
	tests := []compilerTestCase{
		...
		},
		{
			...
			expectedInstructions: []code.Instructions{
				...
				code.Make(code.OpConstant, 1),
				code.Make(code.OpConstant, 2),
				code.Make(code.OpArray, 3), // array of length 3
				code.Make(code.OpPop),
			},
		},
	...
	tests := []compilerTestCase{
		...
		`,
			...
			expectedInstructions: []code.Instructions{
				...
				code.Make(code.OpConstant, 0),
				// 0007
				code.Make(code.OpJump, 11), // jump over the alternative
				// 0010
				code.Make(code.OpNull), // is the alternative when one isn't present in the code
				// 0011
				code.Make(code.OpPop),
				// 0012
				code.Make(code.OpConstant, 1),
...
func TestBooleanExpressions(t *testing.T) {
	tests := []compilerTestCase{
		...
		{ /*
				During compilaiton we can change the order of the operands in order
				to implement < by using greater than.
				1 < 2 == 2 > 1
			*/
			input:             "1 < 2",
			expectedConstants: []interface{}{2, 1},
